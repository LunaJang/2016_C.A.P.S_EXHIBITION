...
#define N_SORT 10	// 소팅함수의 개수
#define N_MENU 3	// 메뉴의 선택지 개수
#define N_SETUP 3	// 설정의 선택지 개수
#define wait_time 2000	// 화면 변경 대기시간

const char* sorting_names[N_SORT] = { "Bubble Sort","Shell Sort","Bogo Sort","Gnome Sort","Quick Sort","Merge Sort","Bitonic Sort","Heap Sort","Select Sort","메뉴로 돌아가기" };
...
// Sorting 함수 추가 방법
// 1. #define으로 선언된 N_SORT의 값을 변경한다.
// 2. 전역 상수로 설정된 베열 sorting_names에 새로운 소팅 함수의 이름을 추가한다. "메뉴로 돌아가기" 직전에 추가하는게 좋음... 아니면 sorting_manager()에서 수정 많이 해야함.
// 3. 새로운 소팅 함수를 "소팅 함수" 단락에 추가한다. 참고로 size는 전역변수로 설정되어있다.
// 4. sorting_manager() 함수의 switch_case 문에 새로운 소팅 함수의 선택지를 추가한다.
// 5. 한번 돌려보고 안돌아가면 연락 ㄱㄱ
...
void sorting_manager() {		// 하드코딩임. 이왕이면 switch-case문 위주로 확인하는걸 추천
